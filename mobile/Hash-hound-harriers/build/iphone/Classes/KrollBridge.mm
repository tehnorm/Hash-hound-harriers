...
@implementation Hash_hound_harriersObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_hash_hound_harriers);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any Hash_hound_harriers patches in JS before a context is loaded... nice for 
...
{
	// create Hash_hound_harriers global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_hash_hound_harriers = [[Hash_hound_harriersObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_hash_hound_harriers];
	
	NSString *_hash_hound_harriersNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_hash_hound_harriersNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_hash_hound_harriers valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.hash_hound_harriers.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
